...
:END:
* TODO Add button to show folding
** TODO Hide folding buttons on subsections empty 
* TODO Add button to open in emacs
** TODO update server to have an endpoint that takes in buffer & timestamp - return none if no change
** TODO refresh code on clock edge
